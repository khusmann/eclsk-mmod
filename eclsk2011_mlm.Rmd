...
#        TATTEN_sc = TATTEN,
      ...
      mutate(
         ...
         OATTEN_F3 = rowMeans(cbind(TBSTNO, 8-TBTRBST, TFOLLOW_sc, TBWTTSK, 8-TBEZDSL,
      ...
      mutate(
         ...
         PINHIB_F4 = rowMeans(cbind(TBSTNO, TBWTTSK, TFOLLOW_sc, TBFLWIN), na.rm=T),
         PATTEN_F3 = rowMeans(cbind(8-TBEZDAC, 8-TBNOFIN), na.rm=T),
...
```{r}
models_read <- list(
  theory = lmer(XRSCALK4 ~ grade + XATTNFS_F3 + XTCHAPP_F1 + XINBCNT_F4 + (1+grade|CHILDID), df,
             REML=F, control=lmerControl(optimizer='bobyqa')),
  
  f3 = lmer(XRSCALK4 ~ grade + OATL_F1 + OENG_F2 + OATTEN_F3 + (1+grade|CHILDID), df,
            REML=F, control=lmerControl(optimizer='bobyqa')),
  
  f4 = lmer(XRSCALK4 ~ grade + MATL_F1 + MENG_F2 + MINHIB_F4 + MATTEN_F3 + (1+grade|CHILDID), df,
             REML=F, control=lmerControl(optimizer='bobyqa')),
  
  f5 = lmer(XRSCALK4 ~ grade + PATL_F1 + PENG_F2 + PINHIB_F4 + PATTEN_F3 + PATTEN2_F5 + (1+grade|CHILDID), df,
             REML=F, control=lmerControl(optimizer='bobyqa'))
)

models_math <- list(
  theory = lmer(XMSCALK4 ~ grade + XATTNFS_F3 + XTCHAPP_F1 + XINBCNT_F4 + (1+grade|CHILDID), df,
             REML=F, control=lmerControl(optimizer='bobyqa')),
  
  f3 = lmer(XMSCALK4 ~ grade + OATL_F1 + OENG_F2 + OATTEN_F3 + (1+grade|CHILDID), df,
            REML=F, control=lmerControl(optimizer='bobyqa')),
  
  f4 = lmer(XMSCALK4 ~ grade + MATL_F1 + MENG_F2 + MINHIB_F4 + MATTEN_F3 + (1+grade|CHILDID), df,
             REML=F, control=lmerControl(optimizer='bobyqa')),
  
  f5 = lmer(XMSCALK4 ~ grade + PATL_F1 + PENG_F2 + PINHIB_F4 + PATTEN_F3 + PATTEN2_F5 + (1+grade|CHILDID), df,
             REML=F, control=lmerControl(optimizer='bobyqa')) 
)
```
...
df %>%
  gather(var, val, XMSCALK4, XRSCALK4, MATL_F1, MENG_F2, MINHIB_F4, MATTEN_F3, XATTNFS_F3, XTCHAPP_F1, XINBCNT_F4) %>%
  ggplot(aes(val)) +
  geom_histogram() +
